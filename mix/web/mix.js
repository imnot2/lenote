    ...
    $.extend(MixClass.prototype, {
        ...
        audio: {
            init: function(_mix){
                ...
                this.audioDom.on('error', function(){
                    ...
                    if(that.audio.currentTime == 0){
                        ...
                        that.audioUI.find('.progress .current').text(that.getDuration(that.audio.currentTime));
                        that.setProgress(that.audio.currentTime);
                        that.audioUI.children('.play').removeClass('light-timeline');
                        that.mix.slider.pointTo(parseInt(that.audio.currentTime));
                    }
                }).on('ended', function(){
                    that.setProgress(150);
            ...
            play: function(_play, noCheck){
                ...
                _play.addClass('stop light-timeline');
                this.audio.play();
                var currentTime = parseInt(this.audio.currentTime);
                if(currentTime == 0){
                    this.setProgress(0);
                    this.audioUI.find('.progress .current').text(this.getDuration(0));
                }
            },
