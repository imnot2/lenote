...
(function () {
    ...
    function addOKListener() {
        dialog.onok = function () {
            ...
            switch (currentTab) {
                case "remote":
                    return insertSingle();                               //插入远程图片选项卡中的图片
                    break;
                case "local":
                    return insertBatch();                               //插入本地上传选项卡中的图片(单张或多张)
                    break;
                case "imgManager":
                    return insertSearch("imageList");           //插入在线管理选项卡中的图片
                    break;
                case "imgSearch":
                    return insertSearch("searchList", true);  //插入图片搜索选项卡中的图片
                    break;
            }
        };
    ...
    function insertBatch() {
        ...
        for (var i = 0, ci; ci = imageUrls[i++];) {
            ...
            tmpObj.islocal_attachment = true;
            //tmpObj.localID = ci.keyID;
            tmpObj.localID = ci.clientResourceID;
            imgObjs.push(tmpObj);
        }
        insertImage(imgObjs);
        hideFlash();
    ...
    function createPreviewImage(url) {
        ...
        var imgTypeReg = /\.(jpg|jpeg|gif|png|bmp|ico|pcx|tiff|dxf|exif)$/gi, //格式过滤
            urlFilter = "";                                     //地址过滤
        ...
        for (var j = 0, length = tabBodys.length; j < length; j++) {
            ...
            if (id != tabSrc) {
                ...
                if (id == "remote") {
                    ...
                    //当图片地址不合法或为空时确定按钮不可用
                    //try{
                    //    if(valid_remote_img(g("url").value)){
                    //        dialog.buttons[0].setDisabled(false);
                    //    }else{
                    //        dialog.buttons[0].setDisabled(true);
                    //    }
   ...
   function valid_local_img(){
    ...
    /*验证远程图片地址*/
   function valid_remote_img(){
       try{
           var v = g("url").value;
           v = v.replace(/&nbsp;/igm, '').replace(/\s+/igm, '');
           var filter = /^(http[s]?|ftp):\/\/.+?\.(jpg|jpeg|gif|png|bmp|ico|pcx|tiff|dxf|exif)$/i;
