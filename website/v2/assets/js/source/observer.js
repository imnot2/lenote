...
(function ($) {
    ...
    function dispatch(options) {
        ...
        var defaults = {
            _event: 'click',
            monitor: '',
            actions: ['_show_notebook', '_arrow', '_show_note',
    ...
    lenoteObserver = {
        ...
        categories: function () {
            //展开与收起笔记本（组）
            dispatch({
                monitor: '#sidebar-a .list .icon-arrow, #sidebar-a .list .icon-arrow-gray'
            });

            //监听笔记本（组）之笔记列表加载及边栏side-bar所有菜单的右键功能菜单(笔记本(组)、标签、星标笔记等)
            dispatch({
                monitor: '#sidebar-a .list-categories li, #sidebar-a .icon-action'
            });
        ...
        arrow: function (id) {
            dispatch({
                monitor: '#' + id + ' .icon-arrow, #' + id + ' .icon-arrow-gray'
            });
        },
