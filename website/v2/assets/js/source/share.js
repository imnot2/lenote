                ...
                attachments: {
                    set: function(resources){
                        ...
                        for(var i = 0; i < resources.length; i++){
                           var _data = resources[i], indexof = _data.resourceName.lastIndexOf('.'), filetype = (indexof != -1)? _data.resourceName.substring(indexof) : ''; 
                                 ext = filetype? filetype.replace(/^\./ig, '').toLowerCase() : '', name = _data.resourceName.replace(filetype, '');
                    ...
                    get: function(localID){
                        ...
                        if(attachment){
                            ...
                            if(/\.kk$/i.test($.trim(attachment.title))){
                                ...
                                //publicLink = utils.getDomain() + '/getShareFile/' + utils.generateMixed(6)+ '~' + attachment.clientResourceID + '/' + encodeURIComponent(publicLink) + '/' + attachment.title;
                                var linkKey = publicLink.match(/\/([^\/]+)$/),
                                     linkKey = linkKey.length == 2? linkKey[1] : publicLink.replace(/http:\/\/iocos.lenovows.com\/v2\/tiny\//, '');
                                publicLink = utils.getDomain() + '/getShareFile/' + utils.generateMixed(6)+ '~' + attachment.clientResourceID + '/' + encodeURIComponent($.trim(linkKey)) + '/' + attachment.title;
                            }
            ...
            if(key){
                ...
                _lenote.share.getContent(key, {
                    success: function(respose){
                        var note = respose.note;
                        if(respose.status == 1){                            
                            window.parent.lenoteDom.editor.setContent(note.content);
                            lenoteDom.editor.data.attachments.set(note.resources);
        ...
        insertContent: function(note){
            ...
            if(note.tags && note.tags.length > 0){
                ...
                for(var i = 0; i < note.tags.length; i++){
                    ...
                    var _tag = note.tags[i], name = utils.unhtml($.trim(_tag.tagName || '').replace(/[,，'"`~!@#\$%\^*\(\)_\+\-=\}\{\|\\\/\?\.:\[\]]/igm, '').substr(0, 20));
                    if(!name) continue;
            ...
            if(note.styleType == 201){
                var srcs = note.resources || [];                
                audiosWrap = ['<div class="note-audios clearfix"><span class="tip">音频:</span>'];
                for(var i = 0; i< srcs.length; i++){
                    var src = srcs[i];
                    if(src.resourceType == 521){
                        var resource = lenoteDom.editor.data.attachments.get(src.clientResourceID),
                              _url = /^https?:\/\//i.test(resource.url)? resource.url : utils.getDomain() + resource.url;
                        audiosWrap.push('<div class="audio audio-play" play="0" id=_audio_'+src.clientResourceID+' url='+_url+'><div class="title">'+src.resourceName+'</div>');
                        audiosWrap.push('<div class="info"><span class="size">'+utils.calFileSize(src.size)+'</span>');
                        audiosWrap.push('<span class="time">'+utils.playTimeFormat(src.fullTime)+'</span></div></div>');
                    }
                }
                audiosWrap.push('</div>');  
                if(audiosWrap.length < 3) audiosWrap = '';
            }

            container.append(caption.join(''));
            
            var detail = ['<div class="detail encopy"></div>'];
            container.append(audiosWrap? audiosWrap.join('') + detail.join('') : detail.join(''));
            if(audiosWrap) setTimeout(function(){utils.audios.init();},500);
            this.show();
        },
        show: function(){
