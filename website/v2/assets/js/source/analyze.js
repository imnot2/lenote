...
(function(){
        ...
        decode: function(dom){
            ...
            if(attachment){
                if(lenoteTag.isSupportForWeb(attachment._type) == 'image'){
                    ...
                    return '<img resource="image" key="' + dom.attr('key') + '" src="' + attachment.url + '" onerror="this.src=\'/assets/images/img404.png\'" title="' + attachment.title + '" _src="' + attachment.url + '" ' + _data + '/>'
                    //return '<img resource="image" key="' + dom.attr('key') + '" src="' + attachment.keyID ? attachment.url : "" + '" onerror="this.src=\'/assets/images/img404.png\'" ' + _data + '/>';
                }else{
    ...
    lenoteAnalyze.encode = function(html){
       var dom = filterTo(html);
        ...
        if(!UE.browser.ie || UE.browser.version != 7) _html = utils.filterXSS(_html);
        return _html;
    };
    ...
    lenoteAnalyze.decode = function(html, isEdit){
        ...
        if(!UE.browser.ie || UE.browser.version != 7) html = utils.filterXSS(html);
        ...
        if(isEdit){
            ...
            if(UE.browser.ie && UE.browser.version == 8) res = res.replace(/<\/?ln-[^>]+?>/igm, '');
        }else{
            //显示时的自定义标签转换(含mix)
