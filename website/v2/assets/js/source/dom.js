    ...
    lenoteDom = {
        ...
        editor: {
           ...
           selectionchange: function(eventName, isChange, keyCode){
            ...
            init: function(view){
                ...
                this.editor.ready(function(){
                    $(window).unbind('editor.event.resize').bind('editor.event.resize', function(e){
                        ...
                        //待办自适应高度
                        if(!$('.note-todos').is(':hidden'))$('.note-todos').css('height',utils.calElemHeight('.note-todos'));
                    });
                    $(window).trigger("editor.event.resize");
            ...
            cleanUI: function(){
                ...
                $('#sidebar-c .note-todos').html('');
                $('#sidebar-c .note-audios').html('');
                //隐藏按钮、元信息、编辑器容器、工具条
                utils.hide('#sidebar-c .toolbar .btn');
                utils.hide('#sidebar-c .note-info, #sidebar-c .note-meta');
                $('#sidebar-c .note-detail, #sidebar-c .note-editor, #sidebar-c .note-todos, #sidebar-c .note-audios').hide();
                //清空标题及笔记元信息
            ...
            editUI: function(title, content){
                ...
                $('#sidebar-c .note-editor').show('fast', function(){
                    try{
                        ...
                        //如果是语音笔记，插入附件跟插入图片按钮禁止点击
                        if(lenoteTag.getNoteStyle() == 201){
                          var btn = {
                            "imgBtn" : $(".edui-for-insertimage"),
                            "attachBtn" : $(".edui-for-attachment")
                          };
                          for(var o in btn){
                            //btn[o].remove();
                            //btn[o].children(".edui-default").addClass("edui-state-disabled");
                          }
                        }
                        //编辑器更多工具
                    ...
                    btn.click(function(event){
                        ...
                        function(){
                            ...
                            if(popup.length == 0){
                                ...
                                if(lenoteDom.editor.data.get('shareToken')){
                                    menu.push('<dd data-value="dellink" class="ellipsis"><span>关闭链接</span></dd>');
                                }else{
                                    menu.push('<dd data-value="shareToken" class="ellipsis"><span>公开链接</span></dd>');
                                }
                                ...
                                popup.children('dd').click(function(){
                                    ...
                                    switch(_this.attr('data-value')){
                                        case 'shareToken': lenoteView.note.share();break;
            ...
            insertTitleInput: function(title){
                ...
                div.push('<input type="text" maxlength="100" value="' + $.trim(title) + '" name="note_title_input" id="note_title_input"/></div>');
            ...
            data: {
                ...
                set: function(note){
                    ...
                    container.data('note', {cid: note.categoryID, id: note.noteID, ver: note.noteVersion, content: note.content, shareToken: note.shareToken});
                ...
                tags: {
                    ...
                    build: function(isDisplay){
                        ...
                        if(_tags){
                            for(var key in _tags){
                                ...
                                var item = _tags[key], name = this.getName(utils.html(item.tagName));
                                if(!name) continue;
                    ...
                    set: function(tags){
                        ...
                        for(var i = 0; i < tags.length; i++){
                            var tag = tags[i], name = this.getName(tag.tagName);
                            if(!name) continue;
                            
                            _data[this.getKey(name)] = {tagID: tag.tagID, tagName: name, tagType: tag.tagType, tagIcon: tag.tagIcon};
                        }
                    ...
                    add: function(name){
                        ...
                        _data[key] = {tagName: name, tagType: 0, tagIcon: 0};
                ...
                attachments: {
                    ...
                    upload: function(_data, _type){
                        var info = JSON.parse(_data.info), filetype = _data.filetype || ''; ext = filetype? filetype.replace(/^\./ig, '') : '', timestamp = (new Date()).valueOf(),
                              filesize = info.executionResult[0].size || _data.filesize;
                        _type = _type || 640;
                        var attachment = {
                            title: _data.filename, name: _data.filename.replace(new RegExp(filetype + '$', 'i'), ''), size: filesize, type: filetype, ext: ext,
                            keyID: info.executionResult[0].keyID, clientResourceID: info.executionResult[0].keyID, _type: _type, createTime: timestamp, utime: new Date(timestamp).format('yyyy-MM-dd hh:mm')
                        };
                        //attachment = $.extend(attachment, {sizeof: utils.calFileSize(attachment.size), url: this.download(attachment.title, attachment.keyID)});
                        attachment = $.extend(attachment, {sizeof: utils.calFileSize(attachment.size), url: this.download(attachment.title, attachment.clientResourceID)});
                        
                        this.add(attachment, true);
                        return attachment;
                    },
                    //download: function(title, keyID){
                    download: function(title, clientResourceID){
                        ...
                        //return keyID ? (title.match(/[^\.]+$/)[0]== 'kk' ? '/getShareFile/' : '/download/')+ utils.generateMixed(6)+ '~' + keyID +'/' + title : "undefined";
                        //return '/download/' + utils.generateMixed(6)+ '~' + keyID +'/' + title;
                        return '/download/' + utils.generateMixed(6)+ '~' + clientResourceID +'/' + title;
                    },                    
                    getThumb: function(resource, width, height){
                        ...
                        // });
                        //return '/thumbnail/'+ utils.generateMixed(6)+ '~' + resource.keyID +'/' + resource.resourceName || resource.title;
                        return '/thumbnail/'+ utils.generateMixed(6)+ '~' + resource.clientResourceID +'/' + resource.resourceName || resource.title;
                    },
                    ...
                    build: function(notCheck){ //notCheck: 不检查附件是否存在于内容中用于解决正在自动保存时上传附件丢失的问题
                        ...
                        for(var key in _attachments){
                            ...
                            if(!notCheck && item._type != 521 && !this.isExist(item.clientResourceID)){ //删除data中已不存在的资源信息
                                this.del(item.clientResourceID);
                                continue;
                            }
                            //重新验证资源的类型值
                            ...
                            var _attachment = {
                                clientResourceID: item.clientResourceID, keyID: item.keyID, resourceName: item.title, resourceType: _type, 
                                publicLink: item.publicLink, size: item.size, resourceVersion: item.version, createTime: item.createTime
                            };
                    ...
                    set: function(resources){ //解析成upload一样的属性-通过名称后辍名解析
                        ...
                        for(var i = 0; i < resources.length; i++){
                            var _data = resources[i], indexof = _data.resourceName ? _data.resourceName.lastIndexOf('.') : -1, filetype = (indexof != -1)? _data.resourceName.substring(indexof) : ''; 
                                 ext = filetype? filetype.replace(/^\./ig, '').toLowerCase() : '', name = _data.resourceName ? _data.resourceName.replace(filetype, '') : '';
                            
                            var attachment = $.extend(_data, {
                                clientResourceID: _data.clientResourceID, title: _data.resourceName, name: name, size: _data.size, type: filetype, ext: ext,  /*localID: _data.local_id,*/ _type: _data.resourceType,
                                keyID: _data.keyID, link: _data.publicLink, createTime: _data.createTime, utime: new Date(_data.createTime).format('yyyy-MM-dd hh:mm')
                            });
                            attachment = $.extend(attachment, {sizeof: utils.calFileSize(attachment.size), url: this.download(attachment.title, attachment.clientResourceID)});
                            this.add(attachment);
                        }
                    ...
                    update: function(resources){ //更新resourceID
                        ...
                        for(var i = 0; i < resources.length; i++){
                            var resource = resources[i], key = 'key_' + resource.clientResourceID, item = files[key];
                            if(item){
                                item['clientResourceID'] = resource.clientResourceID;
                    ...
                    add: function(attachment, isNew){
                       ...
                       _attachment['key_' + attachment['clientResourceID']] = attachment;
                       container.data('attachments', $.extend(container.data('attachments') || {}, _attachment));
                       
                       if(isNew){
                           var new_attachments = container.data('new_attachments') || {}, key = 'key_' + attachment['clientResourceID'];
                           new_attachments[key] = {keyID: attachment['clientResourceID']};
                    ...
                    get: function(localID){
                        ...
                        if(attachment){
                            return $.extend(attachment, {
                                ...
                                url: this.download(attachment.title, attachment.clientResourceID, attachment.link, attachment.size)
                            });
                        }
                        return '';
                    },
                    getFirst: function(){
                        ...
                        for(var key in _attachments){
                            var item = _attachments[key];
                            if(this.isExist(item.clientResourceID)){ //检查附件是否存在于正文内容中
                                if($.trim(item.title)){
                                    result = item;
                                    break;
                                }
                    ...
                    getLast: function(){
                        ...
                        for(var key in _attachments){
                            ...
                            if(this.isExist(item.clientResourceID) || item._type == 521){ //检查附件是否存在于正文内容中
                    ...
                    getNew: function(){
                        ...
                        if(new_attachments && !$.isEmptyObject(new_attachments)){
                            ...
                            for(var key in new_attachments){
                                ...
                                if(this.isExist(item.clientResourceID)){
                                    news[key] = item;
                                    res +=  item.keyID + ';';
                                }
                                //}else{
            ...
            //解析语音笔记语音UI
            showAudio: function(sources){
              var noteAudio = $("#sidebar-c .note-audios"),
                  wrap = $("<div></div>"),
                  tip = $("<span class='tip'>音频:</span>");
              //for (var i=0; i<(sources && sources.length ? sources.length : 0); i++){
              for(var i in sources){
                var src = sources[i];
                if(src.resourceType == 521){
                  var audio = $('<div class="audio audio-play" play="0"></div>'),
                      title = $("<div class='title'></div>"),
                      info = $("<div class='info'></div>"),
                      size = $("<span class='size'></span>"),
                      time = $("<span class='time'></span>"),
                      resource = lenoteDom.editor.data.attachments.get(src.clientResourceID),
                      _url = /^https?:\/\//i.test(resource.url)? resource.url : utils.getDomain() + resource.url;
                  
                  audio.attr({'id':'_audio_' + src.clientResourceID,'url':_url});
                  title.html(src.resourceName);
                  size.html(utils.calFileSize(src.size));
                  time.html(utils.playTimeFormat(src.fullTime));

                  info.append(size).append(time);
                  audio.append(title).append(info);
                  noteAudio.append(tip).append(audio);
                }
              }
              if(noteAudio.children(".audio").length > 0){
                noteAudio.show();
                setTimeout(function(){
                  utils.audios.init();
                },500);                              
        ...
        fragment: {
            getShare: function(key){
                ...
                html.push('<tr class="sep"><td><input type="text" class="text shareToken" disabled="disabled" readonly="readonly" value="' + url + '"/></td></tr>');
        ...
        category: {
            ...
            dropdown: {
                ...
                insert: function(mark, id){
                    ...
                    menu.push('<dd data-value="delete" class="ellipsis' + (notebook.data('type') == 0? ' disabled': '') + '"><span>删除</span></dd>');
                    menu.push('</dl>');
            ...
            build: function(category, isTree, paddingLeft, level){
                ...
                var id = this.getDomID(category.categoryID), icon = '';
                
                icon = '<i class="icon icon_notebook ' + this.getIcon(category.categoryIcon) + '"></i>';
                ...
                li.push('<span class="title" _title="' + md5(category.categoryName) + '">' + category.categoryName + '</span>');
                li.push('<span class="count clearfix"><span class="l"></span><span class="c">' + category.categoryNoteCount + '</span><span class="r"></span></span>');
                if(category.categoryType != 0 )li.push('<i class="icon icon-action e_notebook_action dropdown"></i></div>');
                li.push('</li>');
                var _dom = $(li.join(''));
                level = (/^\d+$/.test(level))? parseInt(level) : -1;
                _dom.data({
                    id: category.categoryID, ver: category.categoryVersion, type: category.categoryType, pnode: category.parentNode,
                    name: category.categoryName, num: category.categoryNoteCount, level: level, logoID: category.categoryIcon
                });
                return _dom;
            },
            ...
            recursiveBuild: function(category, paddingLeft, level){
                //屏蔽待办事项
                category['_categoryName'] = category.categoryName;
                category['categoryName'] = utils.unhtml(category.categoryName);
                if(category.categoryName == '待办事项' && category.categoryType == 0) return '';
        ...
        note: {
            ...
            init: function(note){
                var selectedID = lenoteView.dom.category.getSelected(),
                    isTodo = note.styleType == 1000,
                    isAudioNote = (note.styleType && note.styleType == 201) || lenoteTag.getNoteStyle() == 201;
                lenoteDom.editor.cleanUI();
                $('#sidebar-c .note-info div.title').html(UE.utils.unhtml(note.title) || '无标题笔记');
                this.insertDate(note.userCreateTime, note.lastUpdateTime);
                if(selectedID != 'trash') this.insertPublicLink(note.shareToken);
                this.insertTags();
                this.insertCategory(note.categoryID);
                lenoteDom.editor.data.set(note);
                if(isTodo){
                  lenoteDom.todos.setContent(note.resources);
                  utils.show('#sidebar-c .note-info, #sidebar-c .note-meta');
                }else{
                  lenoteDom.editor.showAudio(note.resources || lenoteDom.editor.data.attachments.getAll());
                  lenoteDom.editor.editor.setContent(lenoteAnalyze.decode(note.content ? note.content : '', true));
                ...
                setTimeout(function(){
                    if(selectedID == 'myshare'){
                        ...
                        if(isTodo){
                            lenoteDom.editor.insertBtn.clean().create().edit(true).del().show();
                        }else{
                ...
                if(isEdit){
                    if(tags){
                        for(var key in tags){
                            ...
                            var item = tags[key], name = lenoteDom.editor.data.tags.getName(utils.html(item.tagName));
                            if(!name) continue;
                    ...
                    if(tags){
                        for(var key in tags){
                            ...
                            if(!name) continue;
                            if(item['tagID']){
                                res.push('<a class="tagname" _id="' + item['tagID'] +'" href="javascript:;">' + name + '</a>');
                            }else{
            ...
            getThumb: function(type, resource){
                ...
                switch(type){
                    case -1: break; //类型未识别
                    case 128: src = '/assets/images/attachment/default.png'; _class = 'small'; break; //通用
                    case 768: break; //文字
                    case 896: break; //手写
                    case 1024: break; //涂鸦
                    case 1152: break; //待办
                    ...
                    default:
                        var suffix = ['apk', 'doc', 'pdf', 'ppt', 'rar', 'xls', 'zip', 'txt'], ext = resource.ext || resource.resourceName.substring(resource.resourceName.lastIndexOf('.'));
            ...
            insert: function(note, isprepend, isNew){
                var target = '#sidebar-b .list-notes', id = this.getDomID(note.noteID), tags = note.tags;
                ...
                //if(note.listStyle && note.listStyle.keyID){
                if(note.listStyle && note.listStyle.clientResourceID){
                    var resource = note.listStyle, media = this.getThumb(resource.resourceType, resource);
                ...
                if($(target).data('type') == 'search'){
                    ...
                    if(note.styleType == 1000){//待办

                      var todosSummaryList = note.summary.paragraphStyle ? note.summary.paragraphStyle == 1 ? $("<ul class='disc'></ul>") : $("<ol class='decimal'></ol>") : $("<ul></ul>"),
                          contents = note.summary.contents,
                          len = contents && contents.length ? contents.length : 0,
                          todosWray = $("<div></div>");
                          
                      todosSummaryList.addClass("summary ellipsis");

                      for(var n= 0; n< (len < 2 ? len : 2); n++){
                        var cont = contents[n],
                            todoStyle = cont.style ? cont.style == 1 ? 'lineThrough' : 'bold' : '';

                        todosWray.append(todosSummaryList.append($("<li "+(todoStyle ? 'class="'+todoStyle+'"':'')+">"+utils.filterXSS(cont.content)+"</li>")));
                      }

                      li.push(todosWray.html());
                    }else{
                ...
                if(tags && tags.length){
                    ...
                    for(var i = 0; i < tags.length; i++){
                        ...
                        var _tag = tags[i], name = isNew? _tag.tagName : lenoteDom.editor.data.tags.getName(_tag.tagName);
                        if(!name) continue;
                ...
                _note.data({ver: note.noteVersion, id: _id, ctime: note.createTime});
                _note.children('.marked').click(function(event){
                    try{
                       if(!event.isPropagationStopped()) event.stopPropagation();
                       if(!event.isDefaultPrevented()) event.preventDefault();
                    }catch(e){}
                    if(lenoteView.dom.category.getSelected() == 'trash') return;
                    ...
                    success: function(respose){
                        ...
                        if(_notes && _notes.length > 0){
                            $.each(_notes, function(i, item){
                                item['_title'] = item['title'];
                                item['title'] = utils.unhtml(item['title']);
                                lenoteDom.note.insert(item,false,false);
                            });
                            //显示第一篇笔记
        ...
        //待办
        todos:{
          html:function(sources){
            var ui = [];
            for(var i= 0; i< sources.length; i++){
              var src = sources[i],
                  wrap = $("<div></div>"),
                  div = $("<div class='todo'></div>"),
              ...
              div.addClass(check[""+src.description.checked+""]);
              title.html(utils.filterXSS(src.description.content));
              src.description.startTime && time.html(new Date(src.description.startTime).format("M/dd h:mm"));
              wrap.append(div.append(title).append(time));
              ui.push(wrap.html());
            }
            return ui.join('');
          },
          setContent:function(sources){
            var html = "",
                todos = $('#sidebar-c .note-todos');
            if(sources && sources.length > 0)html = this.html(sources);
            todos.html(html);
            todos.show();
          }
        }
