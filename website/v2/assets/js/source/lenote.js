...
(function(){
   ...
   lenote = function(token, header){
       ...
       //ajax全局变量定义
        $.ajaxSetup({
           ...
           headers: $.extend({'AuthToken': token, 'Collect-Data': utils.getCollectData(token)}, header || {})
        });
   };
   ...
   lenote.prototype = {       
       ...
       category: {
           ...
           //添加新笔记本(组)
           add: function(parentId, title, type, callback){
               /*
                *  params:
                *     categoryLogoId: 用户自选笔记本(组)图标ID
                */
               title = utils.html(title);
               sendRequest('note/createCategory', {categoryName: title, categoryType: type, categoryLogoID: 0}, callback);
           },
           //更新笔记本(组)
           update: function(id, title, parentId, version, callback){
               title = utils.html(title);
               sendRequest('note/updateCategory', {categoryID: id, categoryName: title, parentCategoryID: parentId, categoryVersion: version, categoryLogoID: 0}, callback);
           },
           //删除笔记本(组)
           del: function(id, version, callback){
               //version = parseInt(version);
               sendRequest('note/deleteCategory', {categoryID: id, categoryVersion: version}, callback);
           }
       },
       ...
       star: {
          ...
          mark: function(id, isMarked, callback){
              if(!id) return false;
              sendRequest('note/markNote', {noteID: id, isMarked: isMarked || false}, callback, false, true, 'note/mark/' + id); //同一篇笔记只允许有一个请求存在
          },
           /*
            *   获取所有星标笔记
            *   params:
           ...
           getAll: function(pagination, summaryCount, callback){
               ...
               sendRequest('note/getMarkedList', {start: pagination.skip, size: pagination.size, summaryCharCount: summaryCount}, callback, false, false, 'note/getNoteList');
           }
       },
       ...
       share: {
           ...
           create: function(id, callback, type, expire){
               ...
               sendRequest('share/shareNote', {noteID: id, expireTime: expire}, callback);
           },
           del: function(id, callback){
               if(!id) return;
               sendRequest('share/disableShareNote', {noteID: id}, callback);
           },
           get: function(id, callback, type){
               if(!id) return;
               sendRequest('share/getShareLinkId', {noteID: id}, callback);
           },
           getContent: function(key, callback, path){
               if(!key) return;
               sendRequest('share/getShareNoteByKey', {shareToken: key}, callback);
           },
           getAll: function(pagination, summaryCount, callback){
               if(typeof summaryCount != 'number') summaryCount = 100;
               sendRequest('note/getShareNotes', {start: pagination.skip, size: pagination.size}, callback, false, false, 'note/getNoteList');
           }
       },
       ...
       note: {
           ...
           getAll: function(pagination, summaryCount, callback){
               ...
               sendRequest('note/getAllNotes', {start: pagination.skip, size: pagination.size, summaryCharCount: summaryCount}, callback, false, false, 'note/getNoteList');
           },
           //获取笔记本(组)的笔记列表
           getListByCategory: function(cid, pagination, summaryCount, callback, isMultiple){
               if(!cid) return;
               if(typeof summaryCount != 'number') summaryCount = 100;
               sendRequest('note/getNoteListByCategory', {categoryID: cid, start: pagination.skip, size: pagination.size, summaryCharCount: summaryCount}, callback, false, isMultiple);
           },
           //搜索
           ...
           add: function(cid, title, content, isMarked, attachments, callback, tags, styleType){
               ...
               sendRequest('note/createNote', {categoryID: cid, title: title, content: content, isMarked: isMarked, resources: attachments, tags: tags, styleType: styleType, summary: summary}, $.extend({
           ...
           update: function(cid, id, title, content, version, attachments, callback, tags, styleType){
               ...
               var summary = utils.html(utils.strip_tags(content)).substr(0, 100);
               sendRequest('note/updateNote', {categoryID: cid, noteID: id, title: title, content: content, noteVersion: version, resources: attachments, tags: tags, styleType: styleType, summary: summary}, $.extend({
                   failure: error_callback,
                   error: error_callback
               }, callback));
           },
           ...
           del: function(id, version, callback){
               var error_callback = function(respose){
                   ...
                   noteData.ver = respose.noteVersion;
                   $.smartDoing({action: 'del', text: '删除笔记失败，请刷新后重试！', statusClass: 'error'});
               };
               sendRequest('note/deleteNote', {noteID: id, noteVersion: version}, $.extend({
                   failure: error_callback,
                   error: error_callback
               }, callback));
           }
       ...
       trash: {
           ...
           del: function(notes, callback){
               sendRequest('note/removeRecycleNote', {notes: notes}, callback);
           },
           //清空回收站
           clear: function(callback){
               sendRequest('note/cleanRecycleBin', {}, callback);
           },
           //还原/恢复笔记:notes: [{noteID: id, version: version},]
           restore:function(notes, callback){
               sendRequest('note/recoverRecycleNote', {notes: notes}, callback);
           }
       }
   };
})();