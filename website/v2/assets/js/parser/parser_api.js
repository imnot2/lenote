...
(function(){
    ...
    function getPlayer(){
        ...
        return window.parent.getPlayer(window);
    }
    ...
    supernote_audio.init = function(maxWidth){
        ...
        for(var i = 0; i < audios.length; i++){
            ...
            try{
                ...
                //console.info(getPlayer().load);
                getPlayer().load(play.attr('id'), play.attr('url'), type, window.parent.user.getUState('t'));
            }catch(e){
                //console.info(e);
            }
        }
    ...
    supernote_audio.startOrPause = function(_audio){
        ...
        if(isPlay == 0){
            ...
            //播放
            try{
              var res = getPlayer().audioPlay(_this.attr('id'), _this.attr('url'), type, window.parent.user.getUState('t'));
              switch(parseInt(res)){
                case 0: supernote_audio.stop(_this.attr('id'));alert('音频正在加载, 请稍候点击播放');break;
                case 1: break;
                case -1: supernote_audio.stop(_this.attr('id'));alert('音频加载失败，请重新加载笔记！');break;
              }
            }catch(e){
                //console.info(e);
            }
