...
remove_html_annotate() {
    ...
    do 
        ...
        then
            ...
            #sed -i '/<!--.\+\?-->/d' $file
            #sed -i '/<!--.\+\?-->//g' $file
            if [ "$tmp" = "package" -a "$filename"x = "index.html"x ]; then
                echo 'Ingore:' $file
                sed -i -e '/^\s*$/d' -e "s/\.\(js\|css\)\(\s*\?[\'\"]\)/.\1?v${Version}\2/g" $file
            else
...
obfuscate_js() {
    ...
    jsfile=( 'source/lib/jquery.js' 'source/lib/xss.js' 'source/lib/jquery.cookie.js' 'source/lib/jquery.lenote.js' 'source/config.js' 'source/utils.js' 'parser/parser.js' 'parser/swfobject.js' 'source/analyze.js' 'source/user.js' 'source/lenote.js' 'source/tag.js' 'source/share.js' )
    :>$targetFile
    for file in ${jsfile[@]}; do cat "$baseURL/$file" >> $targetFile;done
}
