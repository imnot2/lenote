...
(function(){
    ...
    Parser.prototype = {
        ...
        buildAudio: function(tag){
            ...
            res.push('<span class="size">' + resource.fSize + '</span>');
        ...
        analyzeMix: function(){
             ...
             for(var i = 0; i < tags.length; i++){
                ...
                if(tag.attr('type_').toLowerCase() == 'photo'){
                    photos.push(tag);
                    var _resource = this.getResource(tag.attr('key')), phototime = (typeof _resource == 'object')? _resource.startTime : -1;
                }else{
                ...
                while(true){
                    ...
                    if(_tag && _tag.length > 0 && (!_tag.attr('mkey') || _tag.attr('mkey') == last_mkey) && (_tag.attr('mkey') || this.isAcross(_tag, last_mkey))){
                        ...
                        if(_tag.attr('type_') && _tag.attr('type_').toLowerCase() == 'photo'){
                            var _resource = this.getResource(the_double.attr('key')), _phototime = (typeof _resource == 'object')? _resource.startTime : -1;
