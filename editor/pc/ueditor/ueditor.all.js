...
var domUtils = dom.domUtils = {
    ...
    getElementsByTagName:function (node, name,filter) {
        ...
        for(var n = 0,ni;ni=name[n++];){
            var list = (node?node:document).getElementsByTagName(ni);
            for (var i = 0, ci; ci = list[i++];) {
                if(!filter || filter(ci))
                    arr.push(ci);
            }
    ...
    getComputedStyle:function (element, styleName) {
        ...
        if (browser.ie && browser.version < 9 && styleName == 'font-size' && !element.style.fontSize &&
            ...
            //element.removeChild(span);//这样写会出现 #1471 错误
        ...
        applyInlineStyle:function (tagName, attrs, list) {
            ...
            var bookmark = this.createBookmark(),
                ...
                filterFn = function (node) {
                    var b=node.nodeType == 1?node.tagName.toLowerCase() == 'br'||node.tagName.toLowerCase() == 'input':domUtils.isWhitespace(node);
                    return !b;
                },
...
(function () {
    ...
    var Editor = UE.Editor = function (options) {
        ...
        me.setOpt({
            ...
            iframeJsUrl: me.options.UEDITOR_HOME_URL + 'themes/iframe.js',
        ...
        execCommand: function (cmdName) {
            ...
            if (!cmd.notNeedUndo && !me.__hasEnterExecCommand) {
                ...
                if (me.queryCommandState.apply(me,arguments) != -1) {
                    ...
                    if(!me._ignoreContentChange&&!(cmdName=='addfile'||cmdName=='addimage'||range.collapsed)) me.fireEvent('contentchange');
                    me.fireEvent('afterexeccommand', cmdName);
                }
                me.__hasEnterExecCommand = false;
            } else {
                result = this._callCmdFn('execCommand', arguments);
                if(!me._ignoreContentChange &&text!='font'&&cmdName!='selectall') me.fireEvent('contentchange')
            }
...
UE.plugins['removeformat'] = function(){
    ...
    me.commands['removeformat'] = {
        execCommand : function( cmdName, tags, style, attrs,notIncludeA ) {
            ...
            function filterKeyNode(node){
                if(node.getAttribute('key')||node.tagName.toLowerCase()=='form'||node.tagName.toLowerCase()=='input'){
                        return false
                    }else{
                        return true
                }
            ...
            function doRemove( range ) {
                ...
                if ( bookmark.end ) {
                    ...
                    while ( current ) {
                        ...
                        if ( !dtd.$empty[current.tagName.toLowerCase()] && !domUtils.isBookmarkNode( current )&&filterKeyNode(current)) {
                ...
                if(bookmark.end && domUtils.isBlockElm(pN) && !dtd.$tableContent[pN.tagName]&& !dtd.$list[pN.tagName]&&filterKeyNode(pN)){
                    domUtils.removeAttributes(  pN,removeFormatAttributes );
                }
...
//            browser.ie && (cont = cont.replace(/>&nbsp;</g, '><').replace(/\s*</g, '<').replace(/>\s*/g, '>'));
    ...
    var keys = {
            //  /*Backspace*/ 8:1, /*Delete*/ 46:1,
            /*Shift*/ 16:1, /*Ctrl*/ 17:1, /*Alt*/ 18:1,
            37:1, 38:1, 39:1, 40:1,/*Caps*/20:1,/*Window*/92:1,/*Menu*/93:1,/*Esc*/27:1,/*所有屏蔽掉的按键*/'undefined':1
    ...
    me.addListener('keydown', function (type, evt) {
        var me = this;
        var keyCode = evt.keyCode || evt.which; 
        if(evt.shiftKey&&((keyCode>=48&&keyCode<=57)||(keyCode>=186&&keyCode<=192)||(keyCode>=219&&keyCode<=222))) me.fireEvent('contentchange');       
        if(evt.ctrlKey&&keyCode==88)me.fireEvent('contentchange');
...
UE.commands['adjust_paste'] = {
    execCommand: function (command,range){
        var me = this;
        //var range = me.selection.getRange();
        var span=document.createElement('span');
        span.className='paste_keynode';
        ...
        var rangParentStart,rangParentEnd,start=range.startContainer,end=range.endContainer,continuePaste;
        if (start.tagName&&range.startOffset<6) {            
            rangParentStart = findParent(start);   
        }
        if(!range.collapsed){
            if(end.tagName&&range.endOffset<6){
                rangParentEnd = findParent(end);  
            }  
        }
        // me.body.appendChild(document.createTextNode(range.collapsed));
        // me.body.appendChild(document.createTextNode(start.tagName?start.tagName:start.nodeType));
        // me.body.appendChild(document.createTextNode(range.startOffset));
        // me.body.appendChild(document.createTextNode(end.tagName?end.tagName:end.nodeType));
        // me.body.appendChild(document.createTextNode(range.endOffset));
        // me.body.appendChild(document.createTextNode(!rangParentStart));
        // me.body.appendChild(document.createTextNode(!rangParentEnd+"  "));
        //选区的endContainer和startContainer右侧不要紧挨着附件      
        if(rangParentStart){
            if(range.startOffset<=2){
                rangParentStart.parentNode.insertBefore(span,rangParentStart);
                range.collapsed?range.setEndBefore(rangParentStart.previousSibling).collapse(false).select(true):range.setStartBefore(rangParentStart.previousSibling);
            }else{
                rangParentStart.nextSibling?rangParentStart.parentNode.insertBefore(span,rangParentStart.nextSibling):rangParentStart.parentNode.appendChild(span);
                range.collapsed?range.setStartAfter(rangParentStart).collapse(true).select(true):range.setStartAfter(rangParentStart);        
            }            
        }
        if(rangParentEnd){ 
            if(range.endOffset<=2){
                rangParentEnd.parentNode.insertBefore(span,rangParentEnd);
                range.collapsed?range.setEndBefore(rangParentEnd.previousSibling).collapse(true).select(true):range.setEndBefore(rangParentEnd.previousSibling);
            }else{
                rangParentEnd.nextSibling?rangParentEnd.parentNode.insertBefore(span,rangParentEnd.nextSibling):rangParentEnd.parentNode.appendChild(span);
                range.collapsed?range.setStartAfter(rangParentEnd).collapse(true).select(true):range.setEndAfter(rangParentEnd);      
            }           
        } 
        //if(!range.collapsed&&start.tagName=='P'&&end.tagName=='P'&&!rangParentStart&&!rangParentEnd&&range.startOffset<2&&range.endOffset<2) continuePaste=true;      
        if(!range.collapsed&&!rangParentEnd){ 
            var bookmark_for_paste=range.createBookmark(),end=bookmark_for_paste.end,next=end.nextSibling;
            if(next&&next.tagName&&next.getAttribute('key')){
                end.parentNode.insertBefore(span,next);
                range.setEndBefore(next); 
            }              
            range.moveToBookmark(bookmark_for_paste);
        }
        if(!range.collapsed&&!rangParentStart){ 
            var bookmark_for_paste=range.createBookmark(),start=bookmark_for_paste.start,next=start.nextSibling;
            if(next&&next.tagName&&next.getAttribute('key')){
                start.parentNode.insertBefore(span,start);   
                range.setStartBefore(next); 
            }
            range.moveToBookmark(bookmark_for_paste);
        }
        if(rangParentStart||rangParentEnd)range.deleteContents();
        return range;
    }
}
///import core
...
UE.plugins['paste'] = function () {
    ...
    function del_Paste_Keynode(){
        var paste_keynode=$(me.body).find('span.paste_keynode');
    ...
    function getClipboardData(callback) {
        ...
        me.execCommand('adjust_paste',range);
    ...
    function filter(div) {
        ...
        if (div.firstChild) {
            ...
            if (me.queryCommandState('pasteplain') === 1) {
                ...
                var range=me.selection.getRange();
                address = range.createAddress(true);
                me.execCommand('adjust_paste',range);
    ...
    me.addListener('keydown', function (type, evt) {
        ...
        if (keyCode == 13 && !evt.shiftKey) {//回车
            ...
            if(parent && parent.tagName != 'PRE' && !li){
                ...
                //2013.12.24去掉在文本输入 1.XXX 后自动添加有序列表的功能
    ...
    me.addListener('keyup', function(type, evt) {
        ...
        if (keyCode == 13) {
            ...
            if (!browser.ie) {
                ...
                // var range = me.selection.getRange(),aNode=document.createElement('area');
                // if(tag=='p')range.insertNode(aNode);                
            }
        }
    });
    ...
    Popup.prototype = {
        ...
        getHtmlTpl: function (){
            return '<div id="##" class="edui-popup %%" onmousedown="return false;">' +
                ...
                ' <iframe style="position:absolute;z-index:-1;left:0;top:0;background-color: transparent;" frameborder="0" width="100%" height="0" src="javascript:"></iframe>' +
...
(function () {
    ...
    var btnCmds = ['undo', 'redo', 'formatmatch', 'addimage','addfile',
