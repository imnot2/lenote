...
var domUtils = dom.domUtils = {
    ...
    isEmptyNode:function (node) {
        return !node.firstChild || domUtils.getChildCount(node, function (node) {
            return  !domUtils.isBr(node) && !domUtils.isBookmarkNode(node) && !(domUtils.isWhitespace(node) || domUtils.isEmptyInlineElement(node))            
        }) == 0
    },
    /**
     * 清空节点所有的className
        ...
        applyInlineStyle:function (tagName, attrs, list) {
            ...
            var bookmark = this.createBookmark(),
                ...
                filterFn = function (node) {
                   return node.nodeType == 1 ? !(node.tagName.toLowerCase() == 'br' || node.tagName.toLowerCase() == 'img') : !domUtils.isWhitespace(node);
                },
...
//                });
        ...
        setHeight: function (height,notSetHeight) {
            ...
            this.body.style.height = height - 10 +'px';
        },
...
//            }
        ...
        execCommand: function (cmdName) {
            ...
            //去掉更多工具的选中状态。
            var checked = document.querySelector(".edui-button-checked"),
                show = document.querySelector(".edui-moretoolbox-show");
            if(checked){
...
//            }
        ...
        removeAttr:function(attrs){
            ...
            var attrs = attrs.replace(/\s+/g,'-').split('-');
                ...
                for (var p in node.attrs) {
                    switch (p) {
                        case 'size':
                            ...
                            switch (si){
                                case '1' :
                                    cssStyle.push('font-size:10px;');
                                break;
                                case '2' :
                                    cssStyle.push('font-size:13px;');
                                break;
                                case '3' :
                                    cssStyle.push('font-size:16px;');
                                break;
                                case '4' :
                                    cssStyle.push('font-size:18px;');
                                break;
                                case '5' :
                                    cssStyle.push('font-size:24px;');
                                break;
                                case '6' :
                                    cssStyle.push('font-size:32px;');
                                break;
                                case '7' :
                                    cssStyle.push('font-size:48px;');
                                break;
                            }  
                            break;
                        case 'color':
    ...
    function UndoManager() {
        ...
        this.restore = function () {
            ...
            try{
                ...
                if(rng.startContainer) rng.select(noNeedFillCharTags[rng.startContainer.nodeName.toLowerCase()]);
            }catch(e){}
...
//            browser.ie && (cont = cont.replace(/>&nbsp;</g, '><').replace(/\s*</g, '<').replace(/>\s*/g, '>'));
    ...
    function saveScene() {
        ...
        this.undoManger.save(args == 'addfile' || args == 'addimage');
    }
    ...
    me.addListener('keydown', function (type, evt) {
        ...
        if(evt.ctrlKey && keyCode == 88 && !me.selection.getRange().collapsed){
            cutCollapsed = false;
        }
    ...
    me.addListener('keyup', function (type, evt) {
        var keyCode = evt.keyCode || evt.which;
        if(evt.ctrlKey && keyCode == 88 && !cutCollapsed){
            this.undoManger.save(true);
        }
        if (!keys[keyCode] && !evt.ctrlKey && !evt.metaKey && !evt.shiftKey && !evt.altKey) {
            if (inputType)
                return;      
...
UE.plugins['paste'] = function () {
    ...
    function filter(div) {
        ...
        if (div.firstChild) {
            ...
            //140508 by ding：粘贴内容到pre标签里时，粘贴的内容会被选中，手动闭合一下。
...
UE.plugins['list'] = function () {
    ...
    function findPreLi(node){
        while(node && node.tagName && !(node.tagName == "LI" || node.tagName == "UL" || node.tagName == "OL")){
    ...
    me.addListener('keydown', function (type, evt) {
        ...
        if (keyCode == 13 && !evt.shiftKey) {//回车
            ...
            if(parent && parent.tagName != 'PRE' && !li){
                ...
                if(type && !/^\s*1\s*\.\d/i.test(html)){
                    parent.innerHTML = html.replace(type,'');
                    rng.setStartAtLast(parent).collapse(true).select();
                    me.__hasEnterExecCommand = true;
                    me.execCommand('insertorderedlist',tyName[''+type+'']);
                    me.__hasEnterExecCommand = false;
                }
                ...
                if (li) {
                    if (domUtils.isEmptyBlock(li)) {
                        ...
                        if (li !== parentList.lastChild) {
                            ...
                            clearEmptySibling(li,false,true);//只对右边的空节点做处理
                        } else {
...
//                        }
                    ...
                    li = findPreLi(parent);
                    if(li && li.tagName != 'LI'){
                        var _li = range.document.createElement('li');
                        li.nextSibling ? li.parentNode.insertBefore(_li,li.nextSibling) : li.parentNode.appendChild(_li);
                        li = _li;
                    }
                    while(nxt = li.nextSibling){
                        li.appendChild(nxt);
                    }
                    last = findLastNode(li);
                    range.setStart(last,last.nodeName == '#text' ? last.nodeValue.length : 0).collapse(true).select(true);
                }
            ...
            if (range.collapsed && domUtils.isStartInblock(range)) {
                ...
                if (li && domUtils.isStartInblock(tmpRange)) {
                    ...
                    if (start && start !== li.firstChild) {
                        ...
                        // var nextList = start.nextSibling,
                        //     preList = start.previousSibling,
                        //     newli = nextList.getElementsByTagName('li')[0];
                        // newli.firstChild ? newli.insertBefore(start,newli.firstChild) : newli.appendChild(start);
                        // if(domUtils.isEmptyBlock(nextList)) {
                        //     domUtils.remove(nextList);
                        //     var prelast = findLastNode(preList);
                        //     range.setStart(prelast,prelast.tagName ? 0 : prelast.nodeValue.length).setCursor(false,true);
                        // }else{
...
//                                    });
            ...
            queryCommandState:function (command) {
                ...
                try{
                    var list = this.queryCommandValue(command),
                        range = this.selection.getRange(),
                        start = range.startContainer, end = range.endContainer;
                    start = start.nodeType == 1 ? start : start.parentNode;
                    end = end.nodeType == 1 ? end : end.parentNode;
                    if(!list && (start.getAttribute('key') || end.getAttribute('key'))) return -1;
                }catch(e){}
                return 0;
...
(function () {
    ...
    EditorUI.prototype = {
            ...
            return '<div id="##" class="%%">' +
                ...
                (this.toolbars.length ?
                    '<div id="##_toolbarboxouter" class="%%-toolbarboxouter"><div class="%%-toolbarboxinner">' +
                        ...
                        '<div class="edui-for-moretool edui-button-hide" onmousedown="return false;"></div></div><div id="moretoolbox" class="edui-moretoolbox-hide">'+this.showtool().moretool+'</div></div>' : '') +
        ...
        showtool:function(){
            ...
            moretool = document.querySelector("#moretoolbox"),
            html = this.renderToolbarBoxHtml(),
            widthObj = {"fontfamily":85,"fontsize":60,"forecolor":36,"backcolor":36},
            //toolbarW = document.querySelector("#editor").offsetWidth - 4,
            toolbarW = 505,//537 - 6 -26
            childW = 0; 
            
            temp.innerHTML = div.innerHTML = html;

            var divparent = div.querySelector(".edui-toolbar");
            divparent.innerHTML='';

            var parent = temp.querySelector(".edui-toolbar");
            function getWidth(node){
                if(!node) return 0;
                var classname = node.className.replace(/-default|-colorbutton|edui|box|button|split|combox|for-|-|\s/ig,'');
                return widthObj[classname] ? widthObj[classname] : 26               
            }
            while(parent.childNodes.length && (toolbarW - childW) > getWidth(parent.childNodes[0])){
                // console.log(toolbarW);
                // console.log(childW);
                // console.log(getWidth(parent.childNodes[0]));
                var node = parent.removeChild(parent.childNodes[0]);
                divparent.appendChild(node);
                childW += getWidth(parent.childNodes[0]);
            }
            return {
                showtool:div.innerHTML,
                moretool:temp.innerHTML
            }
        },
