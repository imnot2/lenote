...
var browser = UE.browser = function(){
    var agent = navigator.userAgent.toLowerCase(),
        ...
        //allie      : !!window.ActiveXObject || "ActiveXObject" in window, //add ie11
        allie      :  /(msie\s|trident.*rv:)([\w.]+)/.test(agent),
    ...
    browser.gecko =( navigator.product == 'Gecko' && !browser.webkit && !browser.opera && !browser.allie);
        ...
        execCommand: function (cmdName) {
            ...
            if (!cmd.notNeedUndo && !me.__hasEnterExecCommand) {
                ...
                if (me.queryCommandState.apply(me,arguments) != -1) {
                    ...
                    if(!me._ignoreContentChange && (!(cmdName == 'insertimage' || cmdName == 'attachment' || range.collapsed || text == '') || (cmdName == 'insertorderedlist' || cmdName == 'insertunorderedlist' || cmdName == 'removeformat' || cmdName == 'justify'))) me.fireEvent('contentchange');
                ...
                result = this._callCmdFn('execCommand', arguments);
                //!me._ignoreContentChange && me.fireEvent('contentchange');
                if(!me._ignoreContentChange && text != '' && (cmdName == 'underline' && text != 'font')) me.fireEvent('contentchange');
            }
            ...
            //var checked = $(".edui-button-checked"), show = $(".edui-moretoolbox-show");
            //if(checked){
            ...
            //    show.removeClass("edui-moretoolbox-show");
            //}
            return result;
        },
        /**
...
//                img.style.cssText += ';margin:' + (first.margin||0) +"px;" + 'border:' + (first.border||0) +"px solid #000";
            ...
            ci = opt[0];
            if (opt.length == 1) {
                var extend_attr = ci.islocal_attachment? ' resource="image" key="' + ci.localID + '"' : '';
                str = '<img src="' + ci.src + '" ' + (ci._src ? ' _src="' + ci._src + '" ' : '') +
                    ...
                    (ci.vspace && ci.vspace != "0" ? ' vspace = "' + ci.vspace + '"' : '') + extend_attr + ' onerror="this.src=\'/assets/images/img404.png\'"/>';
                ...
                for (var i = 0; ci = opt[i++];) {
                    ...
                    str = '<p ' + (ci['floatStyle'] == 'center' ? 'style="text-align: center" ' : '') + '><img src="' + ci.src + '" ' +
                        ...
                        (ci.border || '') + '" ' +
                        (ci.title ? ' title="' + ci.title + '"' : '') + extend_attr + ' onerror="this.src=\'/assets/images/img404.png\'"/></p>';
                    html.push(str);
                }
            }
...
UE.plugins['link'] = function(){
    ...
    UE.commands['unlink'] = {
        ...
        queryCommandState : function(){
            var link = this.queryCommandValue('link'), flag = true;
            //不能取消资源的链接
            try{
               flag = link && link.getAttribute('resource') ? false : true;
            }catch(e){}
            //return !this.highlight && this.queryCommandValue('link') ?  0 : -1;
            return !this.highlight && link && flag ?  0 : -1;
        }
...
UE.plugins['list'] = function () {
    ...
    me.ready(function () {
        ...
        utils.cssRule('list', 'ol,ul{margin-top:0;margin-bottom:0;pading:0;'+(browser.allie ? '' : 'width:95%')+'}li{clear:both;}'+customCss.join('\n'), me.document);
    });
    //单独处理剪切的问题
    me.ready(function(){
        domUtils.on(me.body,'cut',function(){
...
//                                    range.setEnd(pre, pre.childNodes.length).shrinkBoundary().collapse().select(true);
    ...
    me.commands['insertunorderedlist'] = {
            ...
            queryCommandState:function (command) {
                ...
                try{
                   var link = this.queryCommandValue('link');
                   if(link && link.getAttribute('resource')) return -1;
                }catch(e){}
                try{
                    ...
                    if(!list && (start.getAttribute('_id') || end.getAttribute('_id'))) return -1;
                }catch(e){}
                return 0;
...
(function () {
    ...
    EditorUI.prototype = {
                ...
                editor.addListener('selectionchange', function (t, causeByUi) {
                    ...
                    if (editor.ui._dialogs.linkDialog) {
                        ...
                        if (link && (url = (link.getAttribute('_href') || link.getAttribute('href', 2)))) {
                            //change 当有属性resource="attachment"时不弹出链接修改框
                            if(link.getAttribute('resource')){
                              html = '';
                            }else{
                            var txt = url;
                            if (url.length > 30) {
        ...
        showtool:function(){
            //var temp = document.createElement("div"), div = document.createElement("div"),
            //var temp = $('div'), div = $('div'),
            var temp = $('<div>'), div = $('<div>'),
                  //moretool = document.querySelector("#moretoolbox"), html = this.renderToolbarBoxHtml(),
                  //moretool = $("#moretoolbox"), html = this.renderToolbarBoxHtml(),
                  html = this.renderToolbarBoxHtml(),
                  //widthObj = {"fontfamily":85, "fontsize":60, "forecolor":36, "backcolor":36},
                  //toolbarW = 505, //工具栏初始亮度
                  toolbarW = $('#sidebar-c').width() - 4 - 2, //$('.edui-editor-toolbarboxinner').width()
                  childW = 0,
                  morebuttonW = 4 + 2 + 20 + 6;
            
            if(UE.browser.ie && UE.browser.version === 8) toolbarW -= 100;
            toolbarW -= morebuttonW; //减去morebutton的width + padding + border + maring + position
            //temp.innerHTML = div.innerHTML = html;
            temp.html(html), div.html(html);
            
            //var divparent = div.querySelector(".edui-toolbar");
            //divparent.innerHTML = '';
            var divparent = div.find(".edui-toolbar");
            divparent.html('');
            
            //var parent = temp.querySelector(".edui-toolbar");
            var parent = temp.find(".edui-toolbar"), children = parent.children();
            parent.attr('id', 'edui0');
            ...
            for(var i = 0; i < children.length; i++){
                ...
                if((toolbarW - childW - (UE.browser.ie && UE.browser.version === 8? 10 : 0)) > w || (i === children.length - 1 && w < morebuttonW)){
                    child.remove();
            ...
            //temp.children('.edui-toolbar').attr('id', 'edui0');
            return {
