...
(function(){
	var Util=_tool.Util=function(id){
		this.ue=null;	
		this.arrKey={};		//用于保存当前笔记的所有key，根据这个对笔记中的资源进行判断，看是否为其他笔记复制进来的。
		this.doc=document;
	}
	Util.prototype={
		isIE:function(){
		...
		setCont:function(str){
			...
			for (var i = 0; i < tags.length; i++) {	
				var tag=tags.eq(i);
				this.arrKey[tag.attr('key')]=1;		//创建当前笔记key字典。
		...
		checkKey:function(oDiv){
			...
			for(var i=0;i<tags.length;i++){
				...
				if(key && this.arrKey[key]) this.arrKey[key]=2;		//如果字典里k-v 不为0,则表示这个key检查过一次。
		...
		//附件插入
		//传入clientX，clientY时表示为拖拽上传，反之为控件上传。
