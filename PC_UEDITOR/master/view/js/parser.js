...
(function(){
    ...
    Parser.prototype = {
        ...
        getResource: function(key){
            ...
            //window.external.getResInfo(key)此接口因为不管资源是否存在都有返回值，这里用fUrl属性来区分。
            return resource && resource['fUrl'] ? resource : null;
        },
        ...
        build: function(tag){
            switch(tag.attr('type_').toLowerCase()){
                ...
                case 'audio':
                    var res = this.getResource(tag.getAttribute('key'));
                    if(res && res.fSuffix && res.fSuffix.toLowerCase().substring(1) == 'kk'){
                        tag.replaceWith(this.buildAudio(tag));
                    }else {
                        tag.replaceWith(this.buildAttachment(tag));
                    }
                break;
            }
        },
